...
`scripts/sources.json` is a registry of tracked source URLs (139 as of 2026-09-10 — the original ~64 institutional/athletic sources plus 75 circle sites found via a full web-search pass over `organizations.js`) (club/circle sites, some flagged `jsRendered: true` for sites WebFetch can't read reliably). `scripts/check-sources.js` fetches each one and does *purely mechanical* hash-diff change detection — it does not interpret content. `.github/workflows/check-sources.yml` runs this nightly (03:00 JST), commits the refreshed `sources.json` straight to `main`, and opens/updates a GitHub Issue when something changed. The actual judgment work (reading the change, deciding whether/how to update `events.js`) is deliberately *not* automated further — it's done by running the `.claude/skills/source-check/SKILL.md` skill in an interactive local session, specifically to avoid paying for a separate LLM API call. Don't build automation that calls an external paid LLM API for this project — that tradeoff has already been considered and rejected on cost grounds.

Each source also carries `priority` (`"A"`/`"B"`/`"C"`, editorial value as an event source), `cloudAccessible` (whether *this* environment's outbound-network policy — see Network/environment constraint above — can actually reach the URL; most club sites can, a minority can't), and `preferredReader` (`"cloud"`/`"local"`/`"gpt"` — who should read a flagged change: a cloud Claude Code session, a local Claude Code + Chrome session, or ChatGPT). `check-sources.js`'s summary output includes a "Review Queue" section pre-grouped by `preferredReader` so a human doesn't have to decide per-source which AI to hand a flagged change to — just copy the matching section to whichever one you're using. See `EVENT_REVIEW_RULES.md` for the shared EVENT/NOT_EVENT/REVIEW judgment rules and output format all three should use when interpreting a flagged change.
...
- **Legal guardrails**: read `LEGAL_GUARDRAILS.md` before adding events, collecting information, using images, or writing description text. The two rules that matter most: don't transcribe an external site's text or images just because they're publicly visible, and check `LEGAL_GUARDRAILS.md` before adding an event. Legal judgment calls are risk-rated (🟢/🟡/🟠/🔴/expert-review-recommended), never asserted as definitively legal/illegal — when in doubt, hold off and flag it to the site owner rather than deciding alone.
- **Event review rules**: read `EVENT_REVIEW_RULES.md` when interpreting a `scripts/check-sources.js`-flagged change (or any newly-found source) to decide whether it's an EVENT worth adding, NOT_EVENT, or REVIEW (ambiguous — flag to the site owner rather than deciding alone). This is the editorial-judgment counterpart to `LEGAL_GUARDRAILS.md`'s legal/copyright rules; use both together, regardless of whether the read is done by this cloud session, a local Claude Code + Chrome session, or ChatGPT — same rules, same output format, for consistency across whichever one does the reading.
- Commit/push/merge-to-main without asking each time is allowed **only** for `events.js`/`organizations.js` additions or edits made through an information-gathering pass (e.g. running `scripts/check-sources.js` and then reading the actual source pages) where every field written was directly verified against a primary source — not inferred, not guessed, not carried over from an aggregator summary alone. Regenerate the derived static pages (`generate-event-pages.js`/`generate-org-pages.js`/`generate-sitemap.js`) as part of the same commit. For anything else — code/script changes, refactors, config, or data you couldn't fully verify — never commit, push, or merge without being explicitly asked to in that turn; this remains a firm standing instruction from the site owner (2026-09-06, reaffirmed and scoped 2026-09-07).
- Multiple sessions/people can be editing `events.js`/`organizations.js` concurrently (this has actually happened — see next point), so re-fetch and merge `origin/main` immediately before finalizing any commit that touches these files, not just at the start of a task.
- New `id` values (`evt-XXX`, org ids) are a global counter independent of where in the file the entry lives (entries are grouped by team/organizer in section blocks, not by id or date order — see the file's many `// ---- 団体名 ----` header comments). Because of concurrent edits, a locally-computed "max id + 1" can collide with an id someone else just committed to `main`; this caused a real incident (2026-09-06, evt-252 collision between a manually-added tennis event and an in-progress branch). `generate-event-pages.js`/`generate-org-pages.js` now call `validateUniqueIds()` and exit non-zero on any duplicate id — always run these locally before pushing so a collision is caught immediately rather than silently overwriting a page.
...
- **Match verification effort to actual risk, and always state deployment stage**: a purely cosmetic/low-risk change (an icon, a color) only needs a quick visual/code-level sanity check — running a full multi-scenario Playwright suite for it is disproportionate ("手段の目的化", flagged by the site owner 2026-09-14). Save heavier automated verification for changes that touch data, shared rendering logic, or user-facing behavior across many pages. When reporting a fix, always say which stage it's actually reached — local only, pushed to the branch/PR, or merged to main and live — rather than leaving the site owner to discover a stale deployment on their own. Retrospecting on process (like this bullet) is worth doing when something actually surprised you or the site owner corrected you, not as a fixed per-task ritual — manufacturing a "lesson" after every routine task would be the same disproportion in a different shape.
